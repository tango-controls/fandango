...
public:
  ...
  //---------------------------------------------
  {
    ...
    return bs.to_string();
#else
    //-TODO: fix the following - gcc 3.4.4 and above bug - the following doesn't compile properly
			 //-return bs.to_string<char, std::char_traits<char>, std::allocator<char> >();
			 return std::string("can't convert to string - gcc 3.4.x bug - fix needed");
#endif
  }
...
public:
  ...
  //-------------------------------------------------------------------------
  const bool is_little_endian () const
  //-------------------------------------------------------------------------
  {
...
{\
  ...
  _dest.value() = reinterpret_cast<_T&>(tmp); \
  return _source ; \
} 
//-----------------------------------------------------------------------------
