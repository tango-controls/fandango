...
// ----------------------------------------------------------------------------
template <typename T> 
void get_sock_option (Socket::OSDescriptor _socket_desc, 
                      int _level, 
                      int _opt,
                      T& dest_)
{
	...
	if (::getsockopt(_socket_desc, _level, _opt, (void*)&dest_, &l))
	{
	  THROW_SOCKET_ERROR(err_no, 
                       "OS <getsockopt> call failed", 
                       "Socket::get_option");
    ...
    //- throw (SocketException)
{
	...
	switch(_opt) 
  {
      ...
      {
        BOOL b;
	      get_sock_option(this->m_os_desc, level, native_opt, b);
	      opt_value = stattc_cast<int>(b);
			}
#else 
	    get_sock_option(this->m_os_desc, level, native_opt, opt_value);
#endif
		  break;
		case SOCK_OPT_IBUFFER_SIZE: 
		case SOCK_OPT_OBUFFER_SIZE: 
      ...
      break;
		case SOCK_OPT_LINGER:
		  {
			  struct linger lp;
			  get_sock_option(this->m_os_desc, level, native_opt, lp);
			  if (lp.l_onoff) 
    ...
    case SOCK_OPT_OTIMEOUT:
#ifdef WIN32
      {
	      ...
	      get_sock_option(this->m_os_desc, level, native_opt, opt_value);
			...
			{
				struct timeval * tv;
				get_sock_option(this->m_os_desc, level, native_opt, tv);
			  opt_value = 1000 * tv->tv_sec 
                      ...
                      T& _src)
{
	...
	if (::setsockopt(_socket_desc, _level, _opt, (void*)&_src, l))
	{
	  THROW_SOCKET_ERROR(err_no, 
