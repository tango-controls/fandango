  ...
  throw (SocketException)
{
  ...
  {
    case UDP_PROTOCOL:
      this->m_os_desc = ::socket(AF_INET, SOCK_DGRAM, 0);
      break;
    case TCP_PROTOCOL:
      this->m_os_desc = ::socket(AF_INET, SOCK_STREAM, 0);
      break;
    default:
      THROW_SOCKET_ERROR(GENERIC_SOCKET_ERROR,  
...
// Socket::get_protocol
// ----------------------------------------------------------------------------
Socket::Protocol Socket::get_protocol (void) const
  throw (SocketException)
{
...
// Socket::get_address
// ----------------------------------------------------------------------------
Address Socket::get_address (void) const
  throw (SocketException)
{
  ...
  //-TODO: check this...
  return Address(::inet_ntoa(reinterpret_cast<sockaddr_in*>(&sa)->sin_addr),
...
// Socket::accept
// ----------------------------------------------------------------------------
Socket::OSDescriptor Socket::accept (void)
    throw (SocketException)
{
  YAT_TRACE("Socket::accept");
	...
	OSDescriptor d = ::accept(this->m_os_desc,reinterpret_cast<sockaddr*>(&sa), &l);
    ...
    throw (SocketException)
{
  ...
  CHECK_SOCK_DESC("Socket::connect");
	...
	struct sockaddr_in sa;
	sa.sin_family = AF_INET;
  sa.sin_port = htons(_addr.get_port_number());
    ...
    throw (SocketException)
{
  ...
  if (_tmo_msecs)
  {
    tv.tv_sec = _tmo_msecs / 1000;
    tv.tv_usec = (_tmo_msecs - (tv.tv_sec * 1000)) *  1000; 
  }
    ...
    throw (SocketException)
{
  ...
  CHECK_SOCK_DESC("Socket::get_option");
	...
	int os_opt_value[2] = {0, 0};
	socklen_t l = sizeof(os_opt_value);

	if (
      ::getsockopt(
                   this->m_os_desc, 
                   (_opt == SOCK_OPT_PROTOCOL_TYPE ? IPPROTO_TCP : SOL_SOCKET), 
                   native_opt, 
                   (char*)os_opt_value, 
                   &l
                  )
	...
	switch(_opt) 
  {
    ...
    case SOCK_OPT_PROTOCOL_TYPE:
		...
		case SOCK_OPT_IBUFFER_SIZE: 
		case SOCK_OPT_OBUFFER_SIZE: 
      opt_value = os_opt_value[0];
      break;
		case SOCK_OPT_LINGER:
		{
			//- linger struct does not have the same size on windows and unix.
			struct linger * lp = reinterpret_cast<struct linger*>(os_opt_value);
			if (lp->l_onoff) 
    ...
    case SOCK_OPT_OTIMEOUT:
#ifdef WIN32
	    //- on windows timeout unit is millisecond
			opt_value = static_cast<int>(static_cast<double>(os_opt_value[0]));
#else 
			//- on unix timeout is a timeval
			{
				struct timeval * tv = reinterpret_cast<struct timeval*>(os_opt_value);
				opt_value = 1000 * tv->tv_sec + static_cast<int>(static_cast<double>(tv->tv_usec) / 1000.);
			}
#endif
      break;
		default: 
...
#ifdef WIN32
			l = sizeof(int);
			// on windows, timeout unit is the millisecond
			os_opt_value[0] = _value;
#else
			// on unix, timeout must be specified with timeval structure
			{
        l = sizeof(struct timeval);
				struct timeval* lp = reinterpret_cast<struct timeval*>(os_opt_value);
				lp->tv_sec = _value / 1000;
				lp->tv_usec = (_value - (lp->tv_sec * 1000)) * 1000;
			}
#endif
			break;
		default:
    ...
    throw (SocketException)
{
  ...
  {
    //- read some data
	  rb = ::recv(this->m_os_desc, ib + total_rb, nb - total_rb, 0);
    ...
    throw (SocketException)
{
  ...
  //- send data
	while (btr) 
  {
		int sb = ::send(this->m_os_desc, ob + nb - btr, btr, 0);
		if (sb <= 0) 
    {
      //- operation may block caller (for non blocking socket)
      if (this->current_op_is_blocking())
...
// ----------------------------------------------------------------------------
void Socket::set_blocking_mode (void)
    throw (SocketException)
{
...
// ----------------------------------------------------------------------------
void Socket::set_non_blocking_mode (void)
    throw (SocketException)
{
