...
yat::Message * MessageQ::next_message (size_t _tmo_msecs)
{
  ...
  static bool last_returned_msg_periodic = false;
  ...
  if (! this->wait_not_empty_i(_tmo_msecs))
  {
    ...
    {
      last_returned_msg_periodic = true;
    ...
    {
      last_returned_msg_periodic = false;
      return new Message(TASK_TIMEOUT);
    }
    //- return null msg to caller
    return 0;
  }
  ...
  {
    //- we didn't actually extract the msg from the Q. we just 
    //- "accessed it using "pop_front". since we are about to consume 
    //- the "actual message" to return instead a PERIODIC message, we
    //- have to reinject its presence into the messageQ. quite tricky
    //- isn't it? here the magic line of code...
    //- this->msg_consumer_sync_.signal ();
    last_returned_msg_periodic = true;
    _GET_TIME(this->last_periodic_msg_ts_);
    return new Message(TASK_PERIODIC); 
  }
  //- last returned message was a PERIODIC, return msg currently on top of Q
  else 
  {
    last_returned_msg_periodic = false;
    //... then extract it from the Q and return it
