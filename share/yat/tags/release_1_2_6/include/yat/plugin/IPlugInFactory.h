...
namespace yat
{
  ...
  public:
    ...
    virtual void create(IPlugInObject*& object,
                        const PlugInObjectParams& params = PlugInObjectParams()
                        ) = 0;
...
/*    template <class Interface>
    void create(Interface*& typed_object,
                const PlugInObjectParams& params = PlugInObjectParams())
    {
      IPlugInObject* obj;
      this->create(obj, params);
      try
      {
        typed_object = dynamic_cast<Interface*>(obj);
        if (typed_object == 0)
          throw std::bad_cast();
      }
      catch(std::bad_cast&)
      {
        THROW_YAT_ERROR("BAD_CAST",
                        "The created object is not of the expected type", 
                        "IPlugInFactory::create") ;
      }
    }
*/
