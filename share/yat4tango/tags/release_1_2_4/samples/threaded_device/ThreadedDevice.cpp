...
void ThreadedDevice::delete_device()
{
	...
	if (this->dev_task)
	{
	  ...
	  this->dev_task->exit();
    //- !!!!! NEVER TRY TO <delete> a yat::Task, it commits suicide upon
    //-  return of its main function (i.e. entry point)!!!!!!
	  this->dev_task = 0;
	}
...
void ThreadedDevice::init_device()
{
  ...
  //- instanciate the task
	...
	{
		//- task allocation
		DEBUG_STREAM << "ThreadedDevice::init_device::allocating task" << std::endl;
	  this->dev_task = new MyYatTask(kTASK_PERIODIC_TIMEOUT_MS, this);
	  if (this->dev_task == 0)
      throw std::bad_alloc();
...
void ThreadedDevice::write_postThisValueToTheTask(Tango::WAttribute &attr)
{
  ...
  yat::Message * msg = new yat::Message(kDOUBLE_SCALAR_MSG);
  if (msg == 0)
  {
  ...
  //- in case you want to wait for thr msg to be handled before returning (synchronous approach)
